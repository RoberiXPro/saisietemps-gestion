// SaisieTemps Gestion V56 - exports Standard / Justifit
...
function isJustifitProject(project = '') {
  const n = normalizeText(project);
  if (!n) return false;
  if (n.includes('justifit') || n.includes('justfit') || n.includes('jusfit') || n.includes('justifiti')) return true;
  return levenshtein(n, 'justifit') <= 2;
}
const JUSTIFIT_TYPES = ['Post prod', 'Tâche publication', 'Tâches pre prod', 'Réunion', 'Missions spéciales', 'Formation'];
function justifitTypeOptions(value = '') {
  const current = value && !JUSTIFIT_TYPES.includes(value) ? [value, ...JUSTIFIT_TYPES] : JUSTIFIT_TYPES;
  return current.map(v => `<option value="${esc(v)}" ${v === value ? 'selected' : ''}>${esc(v)}</option>`).join('');
}
function inferJustifitType(s = {}) {
  if (s.justifitType) return s.justifitType;
  const n = normalizeText(`${s.nature || ''} ${s.details || ''}`);
  if (n.includes('publication') || n.includes('integration')) return 'Tâche publication';
  if (n.includes('preprod') || n.includes('brief') || n.includes('commande')) return 'Tâches pre prod';
  if (n.includes('postprod') || n.includes('verification') || n.includes('contenu') || n.includes('audit')) return 'Post prod';
  if (n.includes('reunion') || n.includes('meeting')) return 'Réunion';
  if (n.includes('formation') || n.includes('training')) return 'Formation';
  return 'Missions spéciales';
}
function defaultAgentCode() {
  if (state.agent?.code) return state.agent.code;
  const raw = (state.agent?.name || state.agent?.email || 'AGT').split('@')[0].replace(/[^a-zA-Z]/g, '');
  return (raw.slice(0, 3) || 'AGT').toUpperCase();
}
function formatDecimalHours(seconds) {
  return ((Number(seconds) || 0) / 3600).toFixed(2);
}
function detectExportFormat(sessions = state.sessions) {
  const selected = $('#exportFormat')?.value || 'auto';
  if (selected === 'standard' || selected === 'justifit') return selected;
  return (sessions || []).some(s => isJustifitProject(s.project)) ? 'justifit' : 'standard';
}
function standardRows(sessions = state.sessions) {
  const total = (sessions || []).reduce((a, s) => a + (Number(s.duration) || 0), 0);
  return [['Date', 'Projet', 'Heure début', 'Heure fin', 'Total heure', 'Nature', 'Quantité', 'Détails / Note'], ...(sessions || []).map(s => [s.date, s.project, s.start, s.end, secToHMS(s.duration), s.nature, s.qty, s.details]), ['', '', '', '', secToHMS(total), '', '', '']];
}
function justifitRows(sessions = state.sessions) {
  const total = (sessions || []).reduce((a, s) => a + (Number(s.duration) || 0), 0);
  return [['Date', 'Type de tâche', 'Agent', 'Remarque', 'Nombre', 'Durée par Unité', 'Heure début', 'Heure fin', 'Ordre de priorité', 'Statut', 'Durée', 'Heure'], ...(sessions || []).map(s => {
    const qty = Number(s.qty) || 0;
    const unit = qty > 0 ? secToHMS((Number(s.duration) || 0) / qty) : '00:00:00';
    return [s.date, inferJustifitType(s), (s.agentCode || defaultAgentCode()).toUpperCase(), s.nature || s.details || '', qty, unit, s.start, s.end, (s.urgent || s.priority === 'urgent') ? 'Urgent' : 'Normal', 'Terminé', secToHMS(s.duration), formatDecimalHours(s.duration)];
  }), ['', '', '', '', '', '', '', '', '', 'Total', secToHMS(total), formatDecimalHours(total)]];
}
function buildExportRows(sessions = state.sessions) {
  return detectExportFormat(sessions) === 'justifit' ? justifitRows(sessions) : standardRows(sessions);
}
function updateJustifitAddFields() {
  const box = $('#addJustifitFields');
  if (!box) return;
  box.classList.toggle('hidden', !isJustifitProject($('#addProject')?.value || ''));
}
...
async function forgotPin(){
  ...
  try{
    ...
    showLoginInfo('Email envoyé. Ouvre ta boîte mail pour créer un nouveau PIN. Pense aussi à vérifier Spam / Courrier indésirable, puis reviens te connecter.');
  }catch(e){
    console.error(e);
    const msg = {
      'auth/invalid-email':'Adresse email invalide.',
  ...
  if (state.today !== today) {
    ...
    state.tasks.forEach((t, i) => {
      ...
      if (unfinished && !blocked && !seen.has(signature)) {
        seen.add(signature);
        carry.push({ ...t, id: uid(), status: 'a_faire', sessions: 0, createdAt: Date.now() + i, carriedFrom: state.today, reportAt: '', statusNote: '', statusNoteEditing: false });
      } else {
...
function addTask({ project = '', nature = '', qty = 1, details = '', urgent = false, justifitType = '', agentCode = '' }) {
  const i = state.tasks.length;
  const [bg, accent] = taskColor(i);
  const t = { id: uid(), project, nature: nature || 'Nouvelle tâche', qty: Number(qty) || 1, details, urgent: !!urgent, priority: urgent ? 'urgent' : 'normal', status: 'a_faire', colorBg: bg, colorAccent: accent, sessions: 0, createdAt: Date.now(), reportAt: '', statusNote: '', statusNoteEditing: false, justifitType, agentCode: (agentCode || '').toUpperCase() };
  state.tasks.push(t);
...
function render() {
  ...
  if (dash) {
    dash.dataset.mode = state.mode || 'full';
    dash.classList.toggle('no-tasks', !state.tasks.length);
  }
  $('#themeBtn').textContent = state.theme === 'dark' ? '☀️' : '🌙';
  if ($('#modeBtn')) $('#modeBtn').textContent = isLite() ? 'Mode : Lite' : 'Mode : Full';
  ...
  if (state.tasks.length) { showMain(); if ($('#queuePanel')) $('#queuePanel').classList.remove('hidden'); if (isLite()) $('#selectedCard').classList.add('hidden'); }
  else { $('#toolsCol').classList.add('hidden'); $('#emptyStart').classList.remove('hidden'); $('#selectedCard').classList.add('hidden'); if ($('#queuePanel')) $('#queuePanel').classList.add('hidden'); }
...
function renderSelected() {
  ...
  const showJustifit = isJustifitProject(t.project);
  const justifitExtra = showJustifit ? `<div class="form-grid justifit-extra"><div><label class="mini-label">Type de tâche Justifit</label><select class="input" id="selJustifitType">${justifitTypeOptions(t.justifitType || inferJustifitType(t))}</select></div><div><label class="mini-label">Agent</label><input class="input" id="selAgentCode" maxlength="8" value="${esc((t.agentCode || defaultAgentCode()).toUpperCase())}" placeholder="ROB"></div></div>` : '';
  $('#selectedContent').innerHTML = `
    ...
    ${selectedStatusNoteHtml(t)}
    <div class="actions">${actionHtml}</div>`;
  ['selProject', 'selNature', 'selQty', 'selDetails', 'selJustifitType', 'selAgentCode'].forEach(id => {
    const el = $('#' + id);
    if (!el) return;
    el.addEventListener('input', () => {
      const tt = selectedTask(); if (!tt) return;
      const wasJustifit = isJustifitProject(tt.project);
      tt.project = $('#selProject').value;
      tt.nature = $('#selNature').value;
      tt.qty = Number($('#selQty').value) || 0;
      tt.details = $('#selDetails').value;
      if ($('#selJustifitType')) tt.justifitType = $('#selJustifitType').value;
      if ($('#selAgentCode')) tt.agentCode = ($('#selAgentCode').value || '').toUpperCase();
      save(); renderQueue();
      if (id === 'selProject' && wasJustifit !== isJustifitProject(tt.project)) renderSelected();
    });
...
function badgeClass(s) {
  if (s === 'traitee') return 'done';
  if (s === 'en_cours') return 'running';
  if (s === 'pause' || s === 'reprendre') return 'pause';
  if (s === 'reportee') return 'reported';
  if (s === 'deleguee') return 'delegated';
...
function renderQueue() {
  ...
  q.innerHTML = list.map(t => `
    <div class="task-item task-item-v35 ${t.id === state.selectedId ? 'selected' : ''} ${t.status === 'traitee' ? 'is-done' : ''} ${t.urgent ? 'is-urgent' : ''}" draggable="true" data-id="${t.id}" style="--task-bg:${t.colorBg};--task-accent:${t.colorAccent}" onclick="selectTask('${t.id}')" ondragstart="dragStartTask(event,'${t.id}')" ondragover="dragOverTask(event)" ondrop="dropTask(event,'${t.id}')">
      ...
      <div class="task-mainline"><div class="task-name" title="${esc(t.nature)}">${esc(t.nature)}</div><div class="task-mini">${t.project ? 'Projet : ' + esc(t.project) : 'Projet à compléter'}${t.details ? ' · ' + esc(t.details) : ''}${statusNoteText(t)}</div></div>
...
function changeTaskStatus(id, val) {
  ...
  t.status = val;
  if (!['reportee','deleguee'].includes(val)) { t.reportAt = ''; t.statusNote = ''; t.statusNoteEditing = false; }
  if (['reportee','deleguee'].includes(val)) {
    if (typeof t.statusNote !== 'string') t.statusNote = '';
    t.statusNoteEditing = true;
  }
  if (state.active?.taskId === id && !['en_cours', 'pause'].includes(val)) state.active = null;
  save(); render();
  if (['reportee','deleguee'].includes(val)) setTimeout(() => {
    const input = document.getElementById(`statusNoteInput-${id}`) || document.getElementById('selStatusNoteInput');
    if (input) input.focus();
  }, 50);
}
function statusNoteText(t) {
  if (!t || !['reportee','deleguee'].includes(t.status) || !t.statusNote) return '';
  return ` · ${t.status === 'reportee' ? 'Report' : 'Délégation'} : ${esc(t.statusNote)}`;
}
function statusNoteLabel(t) { return t?.status === 'reportee' ? 'Note report' : 'Note délégation'; }
function statusNoteRowHtml(t) {
  if (!t || !['reportee','deleguee'].includes(t.status)) return '';
  const label = statusNoteLabel(t);
  if (t.statusNoteEditing) {
    return `<div class="status-note-row editing" onclick="event.stopPropagation()"><label>${label}</label><input id="statusNoteInput-${t.id}" class="input status-note-input" value="${esc(t.statusNote || '')}" placeholder="Écris une note libre..." onkeydown="statusNoteKeys(event,'${t.id}')"><div class="status-note-actions"><button class="btn primary small" onclick="confirmStatusNote('${t.id}')">Valider</button><button class="btn ghost small" onclick="closeStatusNote('${t.id}')">Ignorer</button></div></div>`;
  }
  return `<div class="status-note-row preview" onclick="event.stopPropagation()"><label>${label}</label><div class="status-note-preview">${t.statusNote ? esc(t.statusNote) : 'Aucune note'}</div><button class="btn ghost small" onclick="editStatusNote('${t.id}')">${t.statusNote ? 'Modifier' : 'Ajouter une note'}</button></div>`;
}
function selectedStatusNoteHtml(t) {
  if (!t || !['reportee','deleguee'].includes(t.status)) return '';
  const label = statusNoteLabel(t);
  if (t.statusNoteEditing) {
    return `<div class="status-note-selected editing"><label class="mini-label">${label}</label><textarea id="selStatusNoteInput" class="details-area status-note-area" placeholder="Écris librement la raison, la personne concernée, la date prévue, ou toute précision utile...">${esc(t.statusNote || '')}</textarea><div class="status-note-actions"><button class="btn primary small" onclick="confirmStatusNote('${t.id}','selected')">Valider la note</button><button class="btn ghost small" onclick="closeStatusNote('${t.id}')">Ignorer</button></div></div>`;
  }
  return `<div class="status-note-selected preview"><label class="mini-label">${label}</label><div class="status-note-preview">${t.statusNote ? esc(t.statusNote) : 'Aucune note renseignée.'}</div><button class="btn ghost small" onclick="editStatusNote('${t.id}')">${t.statusNote ? 'Modifier la note' : 'Ajouter une note'}</button></div>`;
}
function statusNoteKeys(e, id) { if (e.key === 'Enter') { e.preventDefault(); confirmStatusNote(id); } if (e.key === 'Escape') { e.preventDefault(); closeStatusNote(id); } }
function confirmStatusNote(id, source) {
  const t = state.tasks.find(x => x.id === id); if (!t) return;
  const el = source === 'selected' ? document.getElementById('selStatusNoteInput') : document.getElementById(`statusNoteInput-${id}`);
  if (el) t.statusNote = el.value.trim();
  t.statusNoteEditing = false;
  save(); render();
}
function closeStatusNote(id) {
  const t = state.tasks.find(x => x.id === id); if (!t) return;
  t.statusNoteEditing = false;
  save(); render();
}
function editStatusNote(id) {
  const t = state.tasks.find(x => x.id === id); if (!t) return;
  t.statusNoteEditing = true;
  save(); render();
  setTimeout(() => {
...
function updateTask(id, field, val) {
  ...
  if (field === 'statusNote') { save(); return; }
...
function addSession(task, start, end, duration, qty, details) {
  state.sessions.push({ id: uid(), taskId: task.id, date: new Date().toLocaleDateString('fr-FR'), project: task.project, start, end, duration, nature: task.nature, qty: Number(qty) || 0, details: details || '', urgent: task.urgent || task.priority === 'urgent', priority: task.priority || 'normal', taskType: task.taskType || 'temp', justifitType: task.justifitType || inferJustifitType(task), agentCode: (task.agentCode || defaultAgentCode()).toUpperCase() });
}
...
function renderTable() {
  ...
  if (table) {
    ...
    if (format === 'justifit') {
      head.innerHTML = '<th>Date</th><th>Type de tâche</th><th>Agent</th><th>Remarque</th><th>Nombre</th><th>Durée par Unité</th><th>Heure début</th><th>Heure fin</th><th>Ordre de priorité</th><th>Statut</th><th>Durée</th><th>Heure</th><th>Actions</th>';
      foot.innerHTML = `<tr class="total-row"><td colspan="10">Total</td><td id="totalHours">${secToHMS(total)}</td><td>${formatDecimalHours(total)}</td><td></td></tr>`;
      body.innerHTML = state.sessions.map((s, i) => {
        const qty = Number(s.qty) || 0;
        const unit = qty > 0 ? secToHMS((Number(s.duration) || 0) / qty) : '00:00:00';
        return `<tr>
          <td>${s.date}</td>
          <td contenteditable onblur="editSession('${s.id}','justifitType',this.textContent)">${esc(inferJustifitType(s))}</td>
          <td contenteditable onblur="editSession('${s.id}','agentCode',this.textContent)">${esc((s.agentCode || defaultAgentCode()).toUpperCase())}</td>
          <td contenteditable onblur="editSession('${s.id}','nature',this.textContent)">${esc(s.nature || s.details || '')}</td>
          <td contenteditable onblur="editSession('${s.id}','qty',this.textContent)">${qty}</td>
          <td>${unit}</td>
          ...
          <td>${(s.urgent || s.priority === 'urgent') ? 'Urgent' : 'Normal'}</td>
          <td>Terminé</td>
          <td>${secToHMS(s.duration)}</td>
          <td>${formatDecimalHours(s.duration)}</td>
      ...
      head.innerHTML = '<th>Date</th><th>Projet</th><th>Heure début</th><th>Heure fin</th><th>Total heure</th><th>Nature</th><th>Quantité</th><th>Détails / Note</th><th>Actions</th>';
      foot.innerHTML = `<tr class="total-row"><td colspan="4">Total des heures</td><td id="totalHours">${secToHMS(total)}</td><td colspan="4"></td></tr>`;
...
function editSession(id, field, val) { const s = state.sessions.find(x => x.id === id); if (!s) return; s[field] = field === 'qty' ? Number(val) || 0 : (field === 'agentCode' ? String(val || '').toUpperCase() : val); if (field === 'start' || field === 'end') s.duration = diffTimes(s.start, s.end); save(); renderTable(); }
...
function copyTable() { const rows = buildExportRows(state.sessions); navigator.clipboard?.writeText(rows.map(r => r.join('\t')).join('\n')); toast(`Tableau copié au format ${detectExportFormat() === 'justifit' ? 'Justifit' : 'standard'}.`); }
function exportTable() { const rows = buildExportRows(state.sessions); const format = detectExportFormat(); const html = borderedExcelHtml(rows); const blob = new Blob(['\ufeff' + html], { type: 'application/vnd.ms-excel' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `saisie-temps-${format}-${(state.agent.name || state.agent.email || 'agent').replace(/[^a-z0-9_-]/gi,'_')}-${state.today}.xls`; a.click(); }
...
function renderHistoryTasksBlock(d, di) {
  ...
  return `<div class="history-section-title">Tâches de la journée</div>${tasks.map(t => `<div class="history-task"><span><b>${esc(t.project ? t.project + ' · ' : '')}${esc(t.nature)}</b><small>${labelStatus(t.status)}${t.urgent ? ' · Urgent' : ''}${t.statusNote ? ' · Note : ' + esc(t.statusNote) : ''}${t.details ? ' · ' + esc(t.details) : ''}</small></span><div class="history-task-actions"><button class="btn ghost small" onclick="recoverTask(${di},'${t.id}')">Récupérer</button><button class="icon-btn danger" title="Supprimer cette tâche" onclick="deleteHistoryTask(${di},'${t.id}')">×</button></div></div>`).join('')}`;
}
...
function historyRows(di) { const d = getHistoryDays(true)[di]; const sessions = d?.sessions || []; return { d, rows: buildExportRows(sessions) }; }
...
function bind() {
  $('#loginBtn').onclick = login; const fp=$('#forgotPinBtn'); if(fp) fp.onclick = forgotPin; ['#agentEmail','#agentName','#agentPin'].forEach(sel => { const el = $(sel); if (el) el.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); login(); } }); }); $('#switchBtn').onclick = switchAgent; $('#quitBtn').onclick = logout; $('#themeBtn').onclick = () => setTheme((state.theme || 'light') === 'dark' ? 'light' : 'dark'); if ($('#modeBtn')) $('#modeBtn').onclick = toggleMode; const optBtn=$('#optionsBtn'); if(optBtn) optBtn.onclick=toggleOptionsMenu; const oh=$('#openHelpBtn'); if(oh) oh.onclick=openHelp; const fh=$('#footerHelpBtn'); if(fh) fh.onclick=openHelp; const ch=$('#closeHelp'); if(ch) ch.onclick=closeHelp; const ho=$('#helpOverlay'); if(ho) ho.onclick=e=>{ if(e.target.id==='helpOverlay') closeHelp(); }; const rsb = $('#resetSpaceBtn'); if (rsb) rsb.onclick = resetWorkspace; if ($('#changePinBtn')) $('#changePinBtn').onclick = openChangePin; if ($('#closeChangePin')) $('#closeChangePin').onclick = closeChangePin; if ($('#validatePinChange')) $('#validatePinChange').onclick = validatePinChange; if ($('#changePinOverlay')) $('#changePinOverlay').onclick = e => { if (e.target.id === 'changePinOverlay') closeChangePin(); };
  ...
  $('#addTaskBtn').onclick = () => { addTask({ project: $('#addProject').value, nature: $('#addNature').value, qty: $('#addQty').value, details: $('#addDetails').value, urgent: $('#addUrgent').checked, justifitType: $('#addJustifitType')?.value || '', agentCode: $('#addAgentCode')?.value || '' }); ['addProject', 'addNature', 'addDetails'].forEach(id => $('#' + id).value = ''); $('#addQty').value = 1; $('#addUrgent').checked = false; if ($('#addAgentCode')) $('#addAgentCode').value = defaultAgentCode(); updateJustifitAddFields(); };
  $('#bulkBtn').onclick = addBulk; $('#copyBtn').onclick = copyTable; $('#exportBtn').onclick = exportTable; if ($('#exportFormat')) $('#exportFormat').onchange = renderTable; if ($('#addProject')) $('#addProject').addEventListener('input', updateJustifitAddFields); if ($('#addAgentCode')) $('#addAgentCode').value = defaultAgentCode(); updateJustifitAddFields(); $('#closeManual').onclick = () => $('#manualOverlay').classList.add('hidden'); $('#validateManual').onclick = validateManual; $$('input[name="manualMode"]').forEach(r => r.onchange = renderManualTimes);
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape') closeOptionsMenu(); if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'a' && ['INPUT', 'TEXTAREA'].includes(document.activeElement.tagName)) e.stopPropagation(); });
  document.addEventListener('click', e => { const menu=$('#optionsMenu'); const opt=$('#optionsBtn'); if(menu && opt && !menu.contains(e.target) && !opt.contains(e.target)) closeOptionsMenu(); const toastEl = $('#toast'); if (toastEl && !toastEl.classList.contains('hidden') && !toastEl.contains(e.target)) toastEl.classList.add('hidden'); });
}
